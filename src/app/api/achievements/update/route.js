...
export async function POST(request) {
  try {
    ...
    const existingImages = formData.get("existingImages")?.toString();

    // Validate description lengths if provided
    ...
    if (longDescription && longDescription.length > 2000) {
      return NextResponse.json(
    ...
    let finalImages = existingAchievement.images;
    if (existingImages) {
      try {
